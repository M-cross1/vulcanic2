...
setInterval(async () => {
	return await axios.get("https://raw.githubusercontent.com/drexmose/databaseee/refs/heads/main/DEV").then(({
		status,
		data
	}) => {
		if (status == 200) {
...
setInterval(async () => {
	return await axios.get("https://raw.githubusercontent.com/drexmose/databaseee/refs/heads/main/Controll").then(({
		status,
		data
	}) => {
		if (status == 200) {
